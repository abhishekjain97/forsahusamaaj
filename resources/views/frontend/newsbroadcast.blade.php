			...
			<div class="col-md-12">
                <div class="section-title mb20 text-center">
                    <h1 class="h1"><a href="{{url('/')}}" style="float:left"><i title="Click here for go back screen"
                                class="fa fa-arrow-circle-o-left"></i></a> Sahu Samaj News</h1>
                </div>
            </div>
        </div>
		...
		<div class="row">
            ...
            <div class="col-md-12">
                ...
                <div class="card1 card-primary1">
                    <a class="btn btn-primary" href="javascript:void(0)" style="max-width: 200px;    margin: 10px 20px;" onclick="loadEventDetail(1)">Add News</a>
                    <!-- /.card-header -->
                    <!-- form start -->
                    <form class="card card-primary newform" id="eventDetails1" method="POST" action="{{ route('newsbroadcast') }}" enctype="multipart/form-data" style="display:none;">
                        @csrf
                        <div class="card-body">
                            <div class="row">
							   <div class="col-sm-6">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="col-md-2">
                            ...
                            <button class="btn btn-primary" name="viewAll" value="all" type="submit">View All</button>
                        </div>
                    </div>
    ...
    <div class="events-schedules-area">
        <div class="container">
            ...
            <div class="row justify-content-center flex flex-wrap">
                @for ($i = $start; $i < count($newsbroadcasts); $i++)
                <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4">
                    <div class="single-events-schedules">
                        <div class="events-image">
                            <a href="{{ route('newsbroadcastDetail', $newsbroadcasts[$i]->title) }}">
                                <img style="width:500px; height: 250px"
                                    src="{{asset('uploads/news_broad_cast/'.$newsbroadcasts[$i]->image)}}" alt="image">
                            </a>
                        </div>

                        <div class="events-content">
                            <div><span><i class="fa fa-calendar"></i>
                                {{ date('F d, Y', strtotime($newsbroadcasts[$i]->date)) }}</span></div>
                            <div style="margin-bottom: 15px;"><span><i class="fa fa-map-marker"></i>
                                    @foreach($cities as $city)
                                    <?php if($city->id == $newsbroadcasts[$i]->city_id) {?> {{ $city->city }}, <?php  } ?>
                                    @endforeach

                                    @foreach($states as $state)
                                    <?php if($state->id == $newsbroadcasts[$i]->state_id) {?> {{ $state->name }} <?php  } ?>
                                    @endforeach</span>
                            </div>
                            <h3 class="h3">
                                <a
                                    href="{{ route('newsbroadcastDetail', $newsbroadcasts[$i]->title) }}">{{ Str::limit($newsbroadcasts[$i]->title,30) }}</a>
                            </h3>
                            <p>{!! Str::limit($newsbroadcasts[$i]->description,150) !!}</p>
                        </div>
                    </div>
                </div>
                @endfor
