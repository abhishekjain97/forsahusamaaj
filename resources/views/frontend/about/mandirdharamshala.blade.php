            ...
            <div class="col-md-12">
                <div class="section-title mb20 text-center">
                    <h1 class="h1"><a href="{{url('/')}}" style="float:left"><i title="Click here for go back screen"
                                class="fa fa-arrow-circle-o-left"></i></a> Mandir Dharamshala</h1>
                </div>
            </div>
        </div>
        <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card1 card-primary1">
                            ...
                            <a class="btn btn-primary" href="javascript:void(0)"
                                style="max-width: 240px;    margin: 10px 20px;" onclick="loadEventDetail(1)">Add Mandir
                                Dharamshala</a>
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form class="card card-primary newform" id="eventDetails1" method="POST"
                                action="{{ route('mandirdharamshalaadd') }}" enctype="multipart/form-data">
                                @csrf
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-sm-6">
                            ...
                            @else
                            <a class="btn btn-primary" href="{{route('user_login')}}"
                                style="max-width: 240px;    margin: 10px 20px;">Add Mandir
                                Dharamshala</a>
                            @endif
                        </div>
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="card mb-4">
                            <form method="get" action="{{ route('mandirdharamshala') }}">
                                <!-- @csrf -->
                                @include('frontend.about.searchFieldMandirDharamsala')
                            </form>
                        </div>
                ...
                <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4">
                    <div class="single-events-schedules">
                        <div class="events-image">
                            <a href="{{ route('mandirdharamshalaDetail', $sanghathans[$i]->title) }}">
                                <img style="width:500px; height: 250px"
                                    src="{{ asset('uploads/mandir_dharamshala/' . json_decode($sanghathans[$i]->image)[0]) }}" alt="image">
                            </a>
                        </div>

                        <div class="events-content">
                            <div><span><i class="fa fa-map-marker"></i>
                            {{ $sanghathans[$i]->location }}</span></div>
                            <h3 class="h3">
                                <a
                                    href="{{ route('mandirdharamshalaDetail', $sanghathans[$i]->title) }}">{{ Str::limit($sanghathans[$i]->title,20) }}</a>
                        ...
                        <div class="event-footer">
                            <div class="view_more book-btn text-center">
                                <a href="{{ route('mandirdharamshalaDetail', $sanghathans[$i]->title) }}" class="book-btn-one"><i class="fa fa-angle-right"></i> Read More</a>
                            </div>
                        </div>
                    </div>
                </div>
...
function ImageChange(input) {
    ...
    if (input.files && input.files[0]) {
        for (let index = 0; index < input.files.length; index++) {
            // if value is images
            var file = input.files[index].name;
            var ext = file.split(".");
            ext = ext[ext.length - 1].toLowerCase();
            var arrayExtensions = ["jpg", "jpeg", "png", "bmp", "gif"];
            var exactSize = getFileSize(input.files[index].size);
            ...
            if ($.inArray(ext, arrayExtensions) == -1) {
                ...
                reader.onload = function(e) {
                    ...
                    var exactSize = getFileSize(input.files[index].size);
                    $('.displayImagesShow').append(
                        '<div style="border: 2px solid #dedadad9;padding: 5px;width: 160px;" id="images_remove_'+index+'"><img id="blah'+index+'" src="' +
                        e.target.result +
                        '" alt="your image" width="100%" height="100" /><div id="myProgress"></div><p id="deleteFile" style="margin-top:0;margin-bottom:0;"><span><span id="message" style="color:green;">Success</span><i class="fa fa-trash" onclick="deletefile('+index+')" aria-hidden="true" style="color:green;float:right;margin-top: 4px;"></i></span></p><div id="myBar" style="background-color:green;width:100%;height:10px"></div></div>'
                    );
