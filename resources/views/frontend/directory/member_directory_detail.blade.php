                    ...
                    <div class="col-lg-12">
                        ...
                        <article>
                            ...
                            <header class="mb-4 text-center">
                                <!-- Post title-->
                                <h1 class="fw-bolder mb-1">{{ $data->name }}</h1>
                            </header>
                            <!-- Preview image figure-->
                            <figure class="mb-4"><img class="img-fluid rounded" src="{{ asset('uploads/member_directory/' . $data->profile_photo) }}" alt="..." /></figure>
                            <!-- Post content-->
                            <section class="mb-5">
                                <div><strong>Father's Name:</strong> {{$data->father_name}}</div>
                                <div><strong>Email:</strong> {{$data->email_id}}</div>
                                <div><strong>Country:</strong> {{$data->country->name}}</div>
                                <div><strong>State:</strong> {{$data->state->name}}</div>
                                <div><strong>City:</strong> {{$data->city->city}}</div>
                                <div><strong>Date of Birth:</strong> {{date("d/m/Y", strtotime($data->dob))}}</div>

                                <div><strong>Current Address: </strong> {{$data->current_address}} </div>

                                <div><strong>Permanent Address: </strong> {{$data->permanent_address}} </div>

                                <div><strong>Profession: </strong> {{$data->profession}} </div>

                                <div><strong>Occupation: </strong> {{$data->occupation_type}} </div>
                                <div><strong>Anniversary Date:</strong> {{date("d/m/Y", strtotime($data->marriage_anniversary))}}</div>

                                <div><strong>Address: </strong> {{$data->address}} </div>
                                <div><strong>Highest Education: </strong> {{$data->highest_education}} </div>
                                <div><strong>Blood Group: </strong> {{$data->blood_group}} </div>
                                <div><strong>Description: </strong> {!! $data->description !!} </div>

                                <figure class="mb-4"><img class="img-fluid rounded" src="{{ asset('uploads/member_directory/' . $data->upload_photo) }}" alt="..." /></figure>

                                <figure class="mb-4"><img class="img-fluid rounded" src="{{ asset('uploads/member_directory/' . $data->upload_supporting_doc) }}" alt="..." /></figure>
                            </section>
                        </article>
                        <!-- Comments section-->
                        <!-- <section class="mb-5">
